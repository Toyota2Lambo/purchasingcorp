...
import { sendEmail, inquiryConfirmationEmail } from './_email.js';
...
export default async function handler(req) {
  ...
  // Confirmation email to the customer (best-effort — a failed/omitted send
  // never blocks the lead; only fires when we have an email and Resend is set).
  let emailed = false;
  if (email) {
    const tmpl = inquiryConfirmationEmail({
      device: v.model,
      condition: v.condition,
      offer: firstOffer ? firstOffer.offer : null,
    });
    emailed = await sendEmail({ to: email, subject: tmpl.subject, html: tmpl.html, text: tmpl.text });
  }
